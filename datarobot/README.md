...
- **Full end-to-end re-test pass across all four paths** (after `app.datarobot.com` network access was restored — see Path D below): Path A (`run_local.py`) re-verified with a real prompt, `--json` mode, an empty prompt, an invalid `OPENAI_API_KEY` (confirmed the resulting traceback is DRUM's own expected error-handling behavior, not a bug), an unreachable `MCP_SERVER_URL` (graceful non-fatal fallback, confirmed via log output), and a real live MCP server round-trip (`neo4j-mcp-official` on Cloud Run — genuine `list_tools`/`call_tool` success). Path B (`myagent.py`'s `graph_factory`) was compiled as a LangGraph `StateGraph` and invoked end-to-end with a live `ChatOpenAI` call — confirmed working now that the `--prefer-binary` fix unblocks installing its `litellm`/`langchain` dependencies. Path C's live Cloud Run demo was re-tested for `/readyz`, `/v1/chat/completions`, and three edge cases: malformed JSON body (clean `422` from FastAPI's validator, not a crash), an empty `messages: []` array (`200`, graceful fallback response), and a request with no `Authorization` header (`403` — Google's Cloud Run IAM front-end correctly rejecting it before the app is reached). Path D's full `package`/`validate`/`deploy --dry-run`/`deploy` lifecycle was re-run and is reproducible (the entitlement `422` is a stable account-state condition, confirmed deterministic across repeated runs — not a flaky failure). `agent/model-metadata.yaml`'s lowercase `targetType: agenticworkflow` was cross-checked against DataRobot's own published agent templates (`datarobot-community/datarobot-agent-templates`, `datarobot-agent-application`, and others) and confirmed correct as-is — DRUM's `model-metadata.yaml` schema uses lowercase target-type strings, a different (and unrelated) convention from the REST API's PascalCase `targetType` enum that `infra/agent.py`'s `deploy_to_datarobot()` sends. No further functional bugs were found in this pass; the only remaining gap across all four paths is the DataRobot org entitlement blocker described under Path D below.
...
**Fixed a real dependency conflict**: `requirements-nat.txt` previously pinned `nvidia-nat>=1.8.0`, but every published version of `datarobot-genai[dragent]` (through `0.26.21`, the latest as of this writing) hard-requires `nvidia-nat==1.7.0` exactly — the `>=1.8.0` constraint made `pip install` unsatisfiable (`ResolutionImpossible`) 100% of the time. Fixed by pinning `nvidia-nat==1.7.0` and `nvidia-nat-langchain==1.7.0` to match; re-verified the full install + `nat info components` + `nat run` + `nat mcp serve` flow succeeds with the pin in place.
...
- We do have a `DATAROBOT_API_TOKEN`/`DATAROBOT_ENDPOINT` for a real DataRobot org (used by `infra/agent.py`'s existing manual-deploy path, which predates `task deploy`/Pulumi). With network access to `app.datarobot.com` confirmed working, `python infra/agent.py validate` and `deploy --dry-run` both succeed (`Authenticated as <uid>.`), and running the real `deploy` command against the live org **found and fixed a second real bug**: `POST /customModels/` requires `targetType: "AgenticWorkflow"` (PascalCase — the code sent lowercase `"agenticWorkflow"`, which the API rejects) and a required `customModelType: "inference"` field that was missing entirely. Both are now fixed in `infra/agent.py`.
- **The one thing that could not be completed from here**: after those fixes, `POST /customModels/` for this specific org returns `422 "User does not have access to AgenticWorkflow custom model target type"` — this is a DataRobot account/plan **entitlement**, not a bug in this repo or an environment issue. The org's DataRobot admin/account team needs to enable the `AgenticWorkflow` custom-model target type for this tenant before `infra/agent.py deploy` can create the model and finish the remaining steps (version upload, build, registry, deployment). Once that entitlement is granted, `python infra/agent.py deploy` should complete end-to-end unattended — everything downstream of the `customModels/` create call is already implemented and was exercised (dry-run) against the real, authenticated org.
