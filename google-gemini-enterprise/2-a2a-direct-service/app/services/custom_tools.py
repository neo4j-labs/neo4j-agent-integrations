...
def create_schema_tool(user: str, pwd: str, uri: str, db: str): # -> FunctionTool if you have the import
    ...
    async def get_schema() -> str:
        ...
        """
        # 1. The refined Cypher query from the official neo4j-mcp repository
        query = """
        ...
        try:
            with GraphDatabase.driver(uri, auth=(user, pwd)) as driver:
                ...
                logging.warning(f"Get schema Returned {len(records)} records.")
                if not records:
                ...
                logging.info(f"[CUSTOM TOOLS] Structured schema output prepared with {len(structured_output)} entries.")
                return json.dumps(structured_output, indent=2)

        except Exception as e:
            logging.error(f"[CUSTOM TOOLS] Error fetching schema from {uri}: {e}")
