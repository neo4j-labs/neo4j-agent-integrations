...
A production-ready Next.js chat application demonstrating **NAMS (Neo4j Agent Memory System)** integrated with the **Vercel AI SDK**. Serves as both a working example and a reference client for the [`@neo4j-labs/nams-ai-provider`](../package/vercel-ai-provider) package — persistent memory backed by Neo4j with zero infrastructure to manage.
...
2) cd neo4j-agent-integrations/vercel-agent/vercel_Nams_demo
3) add directly to your `package.json`:"@neo4j-labs/nams-ai-provider": "file:../package/vercel-ai-provider"
...
**NAMS integration lives entirely in `@neo4j-labs/nams-ai-provider`** — installed as a package, not copied source. See [`../package/vercel-ai-provider`](../package/vercel-ai-provider) for the implementation.

---

## Testing the NAMS Provider (Developer Setup)

`vercel_Nams_demo` is the **reference client** for `@neo4j-labs/nams-ai-provider`. It consumes the provider as a local `file:` dependency, so you can make changes to the provider source and test them here without publishing to npm.

### Step 1 — Build the provider

```bash
cd ../package/vercel-ai-provider   # from vercel_Nams_demo/
npm install
npm run build                      # outputs dist/ (JS + types)
```

You only need to rebuild when provider source changes. Run `npm run dev` in `package/vercel-ai-provider/` during active development to rebuild on every save.

### Step 2 — Install in vercel_Nams_demo

`package.json` already has the local link:

```json
"@neo4j-labs/nams-ai-provider": "file:../package/vercel-ai-provider"
```

Install it (from `vercel_Nams_demo/`):
...
readlink node_modules/@neo4j-labs/nams-ai-provider
# ../../../package/vercel-ai-provider
```
...
```bash
# In package/vercel-ai-provider/
npm run build

# In vercel_Nams_demo/ — reinstall to pick up new dist/
npm install --legacy-peer-deps
```
...
`test/` unit-tests the two API routes against **mocked** `@neo4j-labs/nams-ai-provider` exports — no live NAMS/OpenAI credentials needed. These exist specifically to catch the class of break we hit when the provider package was relocated: a route importing a function the new package doesn't export, or a mode wiring the wrong provider call.
...
| File | Covers |
|---|---|
| `test/chat-route.test.ts` | `NAMS_MODE=provider` wraps the model via `createNamsProvider(...).languageModel()`; `NAMS_MODE=tools` sources tools from `createNams().toolsWithMcp()` and leaves the base model unwrapped; MCP-connection failure falls back to NAMS-only tools; `onFinish` persists the step trace via `makeClient()`/`resolveConversation()`; 400/503 error paths |
| `test/reasoning-route.test.ts` | `makeClient()`/`findExistingConversation()` are called with the right config; no-conversation-yet returns `{ steps: [] }` instead of erroring; 400/503/500 error paths |

Run these after any provider upgrade or relocation (as in Step 1) to confirm the demo still speaks the provider's current API before manually clicking through the UI.
...
| `vitest` (dev) | Route-level tests for the NAMS integration (`npm test`) |
...
- [`@neo4j-labs/nams-ai-provider` source](../package/vercel-ai-provider)
