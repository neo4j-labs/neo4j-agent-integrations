...
This repository provides a guided notebook for using CrewAI with Neo4j. It combines CrewAI orchestration, a local Neo4j MCP server, optional Neo4j Agent Memory Server (NAMS), and the repository's reusable graph tools. It does not require a web service or browser application.

## Notebook Workflow

Open and run [`notebooks/crewai_neo4j_walkthrough.ipynb`](notebooks/crewai_neo4j_walkthrough.ipynb). Its sections cover setup, configuration, component concepts, local MCP validation, tool discovery, natural-language graph exploration, and a multi-agent briefing.
...
The notebook includes the equivalent dependency-installation cell. Keep credentials exclusively in `crewai/.env`; do not add them to the notebook or commit that file.
